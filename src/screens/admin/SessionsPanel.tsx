...
function fmtDateTime(iso: string): string { return fmtDate(iso); }
...
export default function SessionsPanel({ exportRef }: { exportRef?: MutableRefObject<(() => void) | null> }) {
  ...
  useEffect(() => subscribe(() => setSyncVersion((v) => v + 1)), []);
          ...
          renderItem={({ item: sess }) => {
            ...
            return (
              ...
              >
                ...
                {diff !== null && (
                  <View style={[s.diffRow, isOver && s.diffOver, isShort && s.diffShort, isExact && s.diffExact]}>
                    <Text style={s.diffLabel}>{isOver ? 'Over' : isShort ? 'Short' : 'Exact'}</Text>
                    <Text style={s.diffValue}>{isExact ? '₱0.00' : `${diff > 0 ? '+' : ''}₱${diff.toFixed(2)}`}</Text>
                  </View>
                )}
              </TouchableOpacity>
            );
...
const s = StyleSheet.create({
  ...
  errorBox: {
    margin: Spacing.xl, backgroundColor: Colors.dangerBg,
    borderRadius: Radius.md, borderWidth: 1,
    borderColor: Colors.danger + '44', padding: Spacing.lg,
  },
  ...
  statusBadge:  { paddingHorizontal: Spacing.md, paddingVertical: Spacing.xs, borderRadius: Radius.full, backgroundColor: Colors.gray100 },
...
const cc = StyleSheet.create({
  cell:  { flex: 1, padding: Spacing.md, alignItems: 'center', borderRightWidth: 1, borderColor: Colors.border },
